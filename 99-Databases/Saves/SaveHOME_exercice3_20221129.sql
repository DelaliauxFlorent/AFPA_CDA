...
--
-- Temporary table structure for view `vw_etudiants_epreuves`
--

DROP TABLE IF EXISTS `vw_etudiants_epreuves`;
/*!50001 DROP VIEW IF EXISTS `vw_etudiants_epreuves`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `vw_etudiants_epreuves` AS SELECT 
 1 AS `idEtudiant`,
 1 AS `nomEtudiant`,
 1 AS `prenomEtudiant`,
...
--
-- Final view structure for view `vw_etudiants_epreuves`
--
...
/*!50001 VIEW `vw_etudiants_epreuves` AS select `et`.`idEtudiant` AS `idEtudiant`,`et`.`nomEtudiant` AS `nomEtudiant`,`et`.`prenomEtudiant` AS `prenomEtudiant`,`et`.`adresseEtudiant` AS `adresseEtudiant`,`et`.`villeEtudiant` AS `villeEtudiant`,`et`.`codePostalEtudiant` AS `codePostalEtudiant`,`et`.`telephoneEtudiant` AS `telephoneEtudiant`,`et`.`dateEntreeEtudiant` AS `dateEntreeEtudiant`,`et`.`anneeEtudiant` AS `anneeEtudiant`,`et`.`remarqueEtudiant` AS `remarqueEtudiant`,`et`.`sexeEtudiant` AS `sexeEtudiant`,`et`.`dateNaissanceEtudiant` AS `dateNaissanceEtudiant`,`et`.`hobby` AS `hobby`,`av`.`idAvoirNote` AS `idAvoirNote`,`av`.`note` AS `note`,`ep`.`libelleEpreuve` AS `libelleEpreuve`,`ep`.`idEnseignantEpreuve` AS `idEnseignantEpreuve`,`ep`.`idMatiereEpreuve` AS `idMatiereEpreuve`,`ep`.`dateEpreuve` AS `dateEpreuve`,`ep`.`CoefficientEpreuve` AS `CoefficientEpreuve`,`ep`.`anneeEpreuve` AS `anneeEpreuve` from ((`avoir_note` `av` join `etudiants` `et` on((`av`.`idEtudiant` = `et`.`idEtudiant`))) join `epreuves` `ep` on((`av`.`idEpreuve` = `ep`.`idEpreuve`))) */;
