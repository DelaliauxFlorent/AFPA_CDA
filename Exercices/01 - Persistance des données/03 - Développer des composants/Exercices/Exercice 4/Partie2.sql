...
SELECT e.nom, e.salaire, p.nom, p.salaire FROM employe AS e INNER JOIN employe AS p ON e.nosup = p.noemp WHERE e.salaire>p.salaire;
...
SELECT nom, titre FROM employe WHERE titre=(SELECT titre FROM employe as e WHERE e.nom="Amartakaldire");
...
SELECT e0.nom, e0.titre FROM employe AS e0 WHERE e0.nodep=31 AND e0.titre IN (SELECT DISTINCT e.titre FROM employe AS e WHERE e.nodep=32);

-- 9 Rechercher le nom et le titre des employés du service 31qui ont un titre l'on ne trouve pas dans le département 32.
SELECT e0.nom, e0.titre FROM employe AS e0 WHERE e0.nodep=31 AND e0.titre NOT IN (SELECT DISTINCT e.titre FROM employe AS e WHERE e.nodep=32);
...
SELECT d.nodept, d.noregion, e.nom FROM dept as d LEFT JOIN employe as e ON d.nodept=e.nodep ORDER BY d.nodept;