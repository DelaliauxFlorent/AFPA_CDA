...
SELECT LEFT(nom,1) FROM employe GROUP BY LEFT(nom,1) HAVING COUNT(*)>=3;

-- 5 Rechercher le salaire maximum et le salaire minimum parmi tous les salariés et l'écart entre les deux.
SELECT MAX(salaire) AS "Salaire Max", MIN(salaire) AS "Salaire Min", MAX(salaire)-MIN(salaire) AS "Écart de salaire" FROM employe;

-- 6 Rechercher le nombre de titres différents.
SELECT COUNT(DISTINCT titre) AS "Nbre de titres" FROM employe;

-- 7 Pour chaque titre, compter le nombre d'employés possédant ce titre.
SELECT titre, COUNT(*) AS "Effectif" FROM employe GROUP BY titre;

-- 8 Pour chaque nom de département, afficher le nom du département et le nombre d'employés.
SELECT d.nom AS "Département", COUNT(*) AS "Effectif" FROM dept AS d LEFT JOIN employe AS e ON d.nodept=e.nodep GROUP BY d.nom;
...
SELECT titre, ROUND(AVG(salaire),2) AS "Salaire moyen du titre" FROM employe GROUP BY titre HAVING AVG(salaire)>(SELECT AVG(salaire) FROM employe WHERE titre ="représentant");

-- 10 Rechercher le nombre de salaires renseignés et le nombre de taux de commission renseignés.
SELECT COUNT(salaire) AS "Nbre salaires renseignés", COUNT(tauxcom) AS "Nbre taux commissions renseignés" FROM employe;